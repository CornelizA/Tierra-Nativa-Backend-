...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        if (packageTravelRepository.count() == 0) {
            ...
            List<PackageTravel> packages = createTravelPackages(categories, chars);

            packageTravelRepository.saveAll(packages);
            packageTravelRepository.flush();
    ...
    private User createDummyReviewer() {
        String email = "viajero.nativo@gmail.com";
    ...
    private Map<String, Category> seedCategories() {
        List<Category> cats = Arrays.asList(
                Category.builder().title("GEOPAISAJES").description("Viajes enfocados en formaciones geológicas, glaciares y paisajes naturales únicos.").imageUrl("https://images.pexels.com/photos/259802/pexels-photo-259802.jpeg").build(),
                Category.builder().title("LITORAL").description("Experiencias en costas, playas, y ecosistemas marinos. Ideal para avistamiento de fauna.").imageUrl("https://images.pexels.com/photos/17676580/pexels-photo-17676580.jpeg").build(),
                Category.builder().title("AVENTURA").description("Actividades de alto impacto y adrenalina como deportes extremos, escalada o rafting.").imageUrl("https://images.pexels.com/photos/13659148/pexels-photo-13659148.jpeg").build(),
                Category.builder().title("ECOTURISMO").description("Exploración y conservación de áreas naturales, priorizando la sostenibilidad y la educación ambiental.").imageUrl("https://images.pexels.com/photos/13197004/pexels-photo-13197004.jpeg").build(),
                Category.builder().title("RELAJACION").description("Destinos y actividades centradas en el bienestar, spa, termas y descanso profundo.").imageUrl("https://images.pexels.com/photos/29581846/pexels-photo-29581846.jpeg").build()
        );
        List<Category> saved = categoryRepository.saveAll(cats);
        categoryRepository.flush();
        return saved.stream().collect(Collectors.toMap(Category::getTitle, c -> c));
    }

    private Map<String, Characteristics> seedCharacteristics() {
        List<Characteristics> chs = Arrays.asList(
                Characteristics.builder().title("Wi-Fi").icon("wifi").build(),
                Characteristics.builder().title("Comida incluida").icon("utensils").build(),
                Characteristics.builder().title("Guías").icon("users").build(),
                Characteristics.builder().title("Seguro de viaje").icon("shield-check").build(),
                Characteristics.builder().title("Equipo").icon("mountain").build(),
                Characteristics.builder().title("Fotos").icon("camera").build(),
                Characteristics.builder().title("Traslados").icon("bus").build(),
                Characteristics.builder().title("Emergencias").icon("first-aid").build(),
                Characteristics.builder().title("Snacks e Hidratación").icon("apple").build(),
                Characteristics.builder().title("Entradas Incluidas").icon("ticket").build(),
                Characteristics.builder().title("Alojamiento Incluido").icon("hotel").build(),
                Characteristics.builder().title("Hotel 5 Estrellas").icon("star").build(),
                Characteristics.builder().title("Refugio de Montaña").icon("mountain-snow").build(),
                Characteristics.builder().title("Mapas y GPS").icon("map").build(),
                Characteristics.builder().title("Vuelos Internos").icon("plane").build()
        );
        List<Characteristics> saved = characteristicRepository.saveAll(chs);
        characteristicRepository.flush();
        return saved.stream().collect(Collectors.toMap(Characteristics::getTitle, c -> c));
    }

    private List<PackageTravel> createTravelPackages(Map<String, Category> cats, Map<String, Characteristics> chars) {
        ...
        paquete2.setCharacteristics(new HashSet<>(Arrays.asList(
                ...
                chars.get("Seguro de viaje"), chars.get("Equipo"), chars.get("Traslados"),
                chars.get("Emergencias"), chars.get("Mapas y GPS"), chars.get("Vuelos Internos"),
                chars.get("Refugio de Montaña")
        )));
        ...
        paquete3.setCharacteristics(new HashSet<>(Arrays.asList(
                ...
                chars.get("Guías"), chars.get("Fotos"), chars.get("Traslados"),
                chars.get("Vuelos Internos"), chars.get("Entradas Incluidas"), chars.get("Hotel 5 Estrellas")
        )));
        ...
        paquete4.setCharacteristics(new HashSet<>(Arrays.asList(
                chars.get("Snacks e Hidratación"), chars.get("Wi-Fi"), chars.get("Comida incluida"),
                chars.get("Guías"), chars.get("Fotos"), chars.get("Traslados"),
                chars.get("Vuelos Internos"), chars.get("Alojamiento Incluido")
        )));
        ...
        paquete5.setCharacteristics(new HashSet<>(Arrays.asList(
                chars.get("Snacks e Hidratación"), chars.get("Comida incluida"), chars.get("Guías"),
                chars.get("Fotos"), chars.get("Traslados"), chars.get("Mapas y GPS"),
                chars.get("Vuelos Internos"), chars.get("Entradas Incluidas"), chars.get("Hotel 5 Estrellas")
        )));
        ...
        paquete6.setCharacteristics(new HashSet<>(Arrays.asList(
                chars.get("Snacks e Hidratación"), chars.get("Comida incluida"), chars.get("Guías"),
                chars.get("Fotos"), chars.get("Traslados"), chars.get("Emergencias"),
                chars.get("Vuelos Internos"), chars.get("Alojamiento Incluido")
        )));
        ...
        paquete8.setCharacteristics(new HashSet<>(Arrays.asList(
                chars.get("Snacks e Hidratación"), chars.get("Comida incluida"), chars.get("Guías"),
                chars.get("Seguro de viaje"), chars.get("Equipo"), chars.get("Fotos"),
                chars.get("Traslados"), chars.get("Emergencias"), chars.get("Mapas y GPS"),
                chars.get("Vuelos Internos"), chars.get("Alojamiento Incluido"), chars.get("Refugio de Montaña")
        )));
        ...
        paquete10.setCharacteristics(new HashSet<>(Arrays.asList(
                chars.get("Snacks e Hidratación"), chars.get("Comida incluida"), chars.get("Guías"),
                chars.get("Fotos"), chars.get("Traslados"), chars.get("Vuelos Internos"),
                chars.get("Alojamiento Incluido")
        )));
    ...
    private void processInteractionsAndScores(List<PackageTravel> packages, User admin, User dummyReviewer) {
        ...
        for (PackageTravel pkg : packages) {
            ...
            int numBlocks = 2 + random.nextInt(3);
            int dayOffset = 5;
            for (int b = 0; b < numBlocks; b++) {
                dayOffset += random.nextInt(8) + 3;
                int duration = random.nextInt(5) + 3;
            ...
            packageTravelRepository.save(pkg);
        }
        packageTravelRepository.flush();
    }
}
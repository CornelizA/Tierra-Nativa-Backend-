...
public class CharacteristicController {
    ...
    public ResponseEntity<CharacteristicDTO> createCharacteristics(@RequestBody Characteristics characteristics) {
        return ResponseEntity.status(HttpStatus.CREATED).body(iCharacteristicService.save(characteristics));
    }

    @PutMapping("/{id}")
    @PreAuthorize("hasAuthority('ADMIN')")
    public ResponseEntity<CharacteristicDTO> update(@PathVariable Long id, @RequestBody Characteristics characteristics) {
        characteristics.setId(id);
        return ResponseEntity.ok(iCharacteristicService.update(characteristics));
    }
